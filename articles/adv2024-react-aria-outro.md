...
今回はまとめ回なので、React Aria での a11y 対応について、いくつかの種類に分けて見ていきます。また、最後には今後の展望なども考察して見ようと思います。
...
これによってスクリーンリーダー利用者は Web サービスを利用できるようになります。しかし、これらの対応はスクリーンリーダー利用者以外にもメリットをもたらします。HTML の正しい構造化や適切なラベル付けによって accessible name が正しく付与されることで、音声コントロールのようなものの利用者は音声によってスムーズに Web サービスを操作できるようになります。適切にラベル付けされたフォーム UI は、特に追加の支援技術を必要としないユーザーに取っても入力するものが分かりやすく、ユーザー体験を向上させることに繋がります。また開発者目線だと、Testing Library などでコンポーネントのテストを書くときに`getByRole`のような関数で対象の要素を取得しやすくなります。
...
キーボード操作のサポートは、視覚障害者の方やマウスを正確に操作することが難しいユーザーにとって使いやすくなるのはもちろん、普段キーボードで様々な操作をしている人にとってもサービスの利用が便利になります。

### モバイル端末での操作

a11y 対応は支援技術を必要とするユーザーのためだけのものではありません。モバイル端末からも Web サービスを利用できる状態というのも、a11y の一種です。
...
今回は範囲外としたのですが、React Spectrum では見た目部分にも責務を持っているので、デザイン上の a11y も行われています。
...
React Aria は実は他の UI フレームワークでも利用されています。
...
書きたかったものが書けたかというと、完全に満足しているわけではないのですが、なんとか一通り React Aria を見ていくことができました。いくつかの`aria-`属性や i18n、ブラウザ・スクリーンリーダーごとに大きく差異があることなど、知らなかったことをたくさん知ることができてよかったです。
